...
### FIXME: Conflicting files from stickynotes_applet
%{__rm} -f %{buildroot}%{_sysconfdir}/gconf/schemas/stickynotes.schemas \
		%{buildroot}%{_libdir}/bonobo/servers/GNOME_StickyNotesApplet.server \
		%{buildroot}%{_libexecdir}/stickynotes_applet
%{__rm} -rf %{buildroot}%{_datadir}/gnome/help/stickynotes_applet/ \
		%{buildroot}%{_datadir}/stickynotes/
...
%{__rm} -rf %{buildroot}%{_datadir}/gnome/help/mailcheck/
...
for file in %{_sysconfdir}/gconf/schemas/*.schemas; do
...
%changelog
* Sun Aug 03 2003 Dag Wieers <dag@wieers.com> - 2.3.6-0
