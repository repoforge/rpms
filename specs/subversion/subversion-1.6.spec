...
#    Requires rpm software with muti-architecture single build support 
...
# Compare to subversion/contrib version
...
%{!?ruby_sitearch: %global ruby_sitearch %([ ! -x %{__ruby} ] || %{__ruby} -rrbconfig -e 'puts Config::CONFIG["sitearchdir"]')}
%endif
...
#Source0: http://archive.apache.org/dist/subversion/subversion-%{version}.tar.bz2
...
# Get updated psvn.el from reliable repo
Source4: http://svn.apache.org/repos/asf/subversion/tags/1.7.0-beta3/contrib/client-side/emacs/psvn.el
...
Patch11: subversion-1.6.15-apr.patch
...
# More general requirement, file-devel not always separate package
...
# which has both kde3 as default
%if %{with_kwallet}
...
License: BSD
# -5: fake release for the Obsoletes to work, synced to version-release
#Provides: svn2cl = %{svn2cl_version}-0.7
#Obsoletes: svn2cl < 0.11-0.6
%if %{with_noarch}
BuildArch: noarch
%endif
Requires: libxslt
...
%{__sed} -i -e 's|^XSL="$dir/|XSL="%{_datadir}/svn2cl/|' \
...
%{__make} swig-py swig-pl swig-pl-lib
%if %{with_ruby}
...
%{__make} install-swig-py %{swigdirs} DESTDIR="${RPM_BUILD_ROOT}"
%{__make} install-swig-pl-lib DESTDIR="${RPM_BUILD_ROOT}"
%if %{with_ruby}
    %{__make} install-swig-rb DESTDIR="${RPM_BUILD_ROOT}"
%endif
...
%{__install} -Dp -m0644 %{SOURCE1} ${RPM_BUILD_ROOT}%{_sysconfdir}/httpd/conf.d/subversion.conf

# Install SysV init script
%{__install} -Dp -m 755 %{SOURCE6} \
...
%if %{with_psvn}
    ...
    %{__install} -Dp -m0644 %{SOURCE4} \
	    ${RPM_BUILD_ROOT}%{_datadir}/emacs/site-lisp/psvn.el
    %{__install} -Dp -m0644 %{SOURCE4} \
	    ${RPM_BUILD_ROOT}%{_datadir}/xemacs/site-packages/lisp/psvn.el

    %{__install} -Dp -m0644 %{SOURCE5} \
	    ${RPM_BUILD_ROOT}%{_datadir}/emacs/site-lisp/psvn-init.el
%endif
...
# Install bash_completion for bash 4.x
%if %{with_bash_completion}
    %{__install} -Dp -m0644 tools/client-side/bash_completion ${RPM_BUILD_ROOT}%{_sysconfdir}/bash_completion.d/%{name}
%endif
...
%files svn2cl
%defattr(-,root,root,-)
%doc contrib/client-side/svn2cl/NEWS
%doc contrib/client-side/svn2cl/README
%doc contrib/client-side/svn2cl/TODO
%doc contrib/client-side/svn2cl/authors.xml
...
* Fri Apr 13 2012 Nico Kadel-Garcia <nkadel@gmail.com> - 1.6.18-0.1
- Update to 1.6.18
- Move 'swig-pl' and 'swig-pl-lib' build out of ruby dependncy for RHEL 4
- Change Source0 URL to subversion.tigris.org

* Mon Apr 09 2012 Nico Kadel-Garcia <nkadel@gmail.com> - 1.6.17-0.4
- Synchronize filter-requires.sh with 1.7.4 SRPM.

* Mon Apr 02 2012 Nico Kadel-Garcia <nkadel@gmail.com> - 1.6.17-0.3
- Integrate Mike Braer's "Plosify" steps to provide RHEL 4 support
- Simplify "_with" settings, actife if defined, inactive if undefined.
- Integrate use of local copies of neon and python, much like local
  copies of aplite, for RHEL 4.
- Simplify bash_completion deployment.
- Simplify docs cleanup.

* Thu Aug 18 2011 Nico Kadel-Garcia <nkadel@gmail.com> - 1.6.17-0.1
- Tweak for RPMforge compatibility of multiple releases
- Roll back release number to avoid conflicts with official releases
- Put BuildRequires each on single line for clarity
- Remove BuildRequires: perl-devel, implied by modules, not part of RHEL 5
- Reduce BuildRequires: apr-devel and apr-util-devel to 1.2.7 for RHEL 5
- Reduce BuildRequies: swig-devel 3.3.6 for RHEL 5
- Add BuildRequires: gcc-c++ for RHEL 5
- Add BuildRequires: qt-devel for RHEL 5 and 6
- Add BuildRequires: /usr/bin/kde4-config for RHEL 6, which has kde and kde4
- Silence calculation of ruby_sitearch if ruby not installed
- Silence failed httpd-mmn testing
- Add _without_psvn to avoid psvn on RHEL 5 with old emacs, xemacs
- Add _without_sqlite to build sqlite-amalgamation instead for RHEL 5
- Add _without_kwallet to avoid KDE4 based packages for RHEL 5
- Add _without_noarch to avoid autoconf problems for RHEL 5
- Use __ln_s instead of __mv for sqlite-amalgamation, to avoid confusion
- Regularize configure elements, fewer quote marks, alphabetize
- Update Source to archive.apache.org stable target
- Provide apache.org URL for version-controlled psvn.el
